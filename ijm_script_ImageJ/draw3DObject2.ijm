...
for (idx =0;idx<embryonames_list.length;idx++){
	...
	embryoname_list = getFileList(input_path);
	write(map_list.length+embryoname_list.length);
	for (i = 0; i < embryoname_list.length; i++){
		if (endsWith(embryoname_list[i], ".tif")) {
			...
			action(input_path, output_path, embryoname_list[i], i,max_index);
