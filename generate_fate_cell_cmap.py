...
res_embryos = {0.25: [],
               0.18: ["191108plc1p1", "200109plc1p1", "200113plc1p2", "200113plc1p3", "200322plc1p2", "200323plc1p1",
                      "200326plc1p3", "200326plc1p4", "200122plc1lag1ip1", "200122plc1lag1ip2"],
               }

save_folder = r"D:\OneDriveBackup\OneDrive - City University of Hong Kong\paper\7_AtlasCell\GUIData\FateCell"
data_folder = r"D:\OneDriveBackup\OneDrive - City University of Hong Kong\paper\7_AtlasCell\GUIData\DeteletedCell"
...
log_file = r"D:\OneDriveBackup\OneDrive - City University of Hong Kong\paper\7_AtlasCell\DatasetUpdated\PairedFate.csv"
...
def change_labels(seg, label2name_dict, cell2fate, fate2label):
    ...
    for label in labels:
        tissue_label = fate2label[cell2fate[label2name_dict[label]]]
...
if COPY_FILE:
    # volume
    for embryo_name in tqdm(embryo_names, desc="Moving files from DeletedCell"):
        file_name = os.path.join(data_folder, embryo_name, embryo_name + "_surface.csv")
        ...
        file_name = os.path.join(data_folder, embryo_name, embryo_name + "_volume.csv")
        pd_data = pd.read_csv(file_name, index_col=0, header=0)
        pd_data = pd_data.applymap(lambda x: "")
        ...
        move_file(os.path.join(data_folder, embryo_name, embryo_name + "_Stat.csv"),
                      os.path.join(save_folder, embryo_name, embryo_name + "_Stat.csv"))

        file_name = os.path.join(save_folder, embryo_name, embryo_name + "_Stat.csv")
        pd_data = pd.read_csv(file_name, index_col=None, header=0)
        pd_data = pd_data.applymap(lambda x: "")
        pd_data.to_csv(file_name, index=False)

        # raw_folder = os.path.join(data_folder, embryo_name, "RawMemb")
        # raw_files = sorted(glob.glob(os.path.join(raw_folder, "*.nii.gz")))
        # seg_folder = os.path.join(data_folder, embryo_name, "SegCell")
