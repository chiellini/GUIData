...
def construct_celltree(nucleus_file, max_time, num_file, read_old=False):
    ...
    if not read_old:
        ...
        df_time = pd.read_csv(nucleus_file, engine='python', delimiter="    ", header=0, index_col=False)
        df_time.columns = ["cell", "time", "z", "x", "y"]
    ...
    #
    # print(len(cell_list),len(cell_in_dictionary),add_cell_list)
    #
    ...
    for cell_name in list(all_cell_names):
        ...
        cell_info = cell_node()
        cell_info.set_number(name2label_dict[cell_name])
